...
export class NocoDbUpsert implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'NocoDb (Upsert)',
		name: 'nocoDbUpsert',
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		if (resource === 'row') {
			...
			if (operation === 'upsert') {
				const primaryKey = this.getNodeParameter('primaryKey', 0, 'id') as string;
				const id = this.getNodeParameter('id', 0) as string; // ↳ unique key value

				// Build payload like in create/update, for the first item only
				const i = 0;
				const newItem: IDataObject = {};
				const dataToSend = this.getNodeParameter('dataToSend', i) as
					| 'defineBelow'
				...
				try {
					// 1️⃣ Try to PATCH (update) first
					await apiRequest.call(
						this,
						'PATCH',
						version === 3
							? `/api/v2/tables/${table}/records/${id}`
							: `/api/v1/db/data/noco/${baseId}/${table}/${id}`,
						payload,
					);
					returnData.push({ ...payload, [primaryKey]: id, _action: 'updated' });
				} catch (err: any) {
					if (err.statusCode !== 404) throw err; // real error – re‑throw

					// 2️⃣ Row is missing → POST to create it
					const created = await apiRequest.call(
						this,
						'POST',
						version === 3
							? `/api/v2/tables/${table}/records`
							: `/api/v1/db/data/noco/${baseId}/${table}`,
						payload,
					);
					returnData.push({ ...created, _action: 'inserted' });
				}
			}
		}
